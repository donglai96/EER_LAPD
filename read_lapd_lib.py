...
def check_devices(fname):
    ...
    if name_check[1]:    # [1] is for SIS crate, check if active
        ...
        for ii in range(len(crate_subgroup_names)):
            ...
            # Search for all config names starting with 'SIS crate' and outputs
            #  digitizer number
            temp = [re.match("SIS crate (\S*)", ii)[1] for ii in
                    config_subgroup_names]
            all_sis_config = ['SIS '+ii for ii in set(temp)]
            config_check = [(item in all_sis_config) for item in devices]

            name_check = [any([ii, jj]) for ii, jj in zip(name_check,
                          config_check)]  # update
...
def lapd_6k_config(fname, motionid=0, quiet=0):
    return lapd_motionlist_config(fname, motionid=motionid,
                                  module='6K Compumotor', quiet=quiet)


def lapd_ni_xz_config(fname, motionid=0, quiet=0):
    return lapd_motionlist_config(fname, motionid=motionid, module='NI_XZ',
                                  quiet=quiet)


def lapd_ni_xyz_config(fname, motionid=0, quiet=0):
    return lapd_motionlist_config(fname, motionid=motionid, module='NI_XYZ',
                                  quiet=quiet)
...
def lapd_motionlist_config(fname, motionid=0, module='6K Compumotor', quiet=0):
    ...
    tbx.qprint(quiet, 'Reading data from motion list {0}'.
               format(motion_dataset_names[motionid]))

    # k:motionid
    motion_data = ff[motion_group_name+motion_dataset_names[motionid]]
    ...
    # set checks for unique elements
    n_motion_lists = len(list(set(motion_list_list)))
    ...
    # Error checking for incomplete datasets
    if len(motion_list_shot) % nxnynz != 0:
...
def get_sis3820_info(fname, sisconfig_name, quiet=0):
    ...
    strings_clock_source = ['Internal 100 MHz (delay locked loop)',
                            '2nd Internal 100 MHz (U580)', 'External Clock',
                            'VME Key Clock']
    strings_clock_mode = ['Double clock',  'Straight clock', '1/2 clock',
                          '1/4 clock', 'User-specified divider']

    clock_odd_output = strings_clock_odd_outputs[ff[sis3820_name].attrs[
        'Odd outputs']]
    clock_even_output = strings_clock_even_outputs[ff[sis3820_name].attrs[
        'Even outputs']]
...
def get_sisboard_config(fname, sisconfig_name, sid=0, quiet=0):
    ...
    sis_config_indices = list(ff[sisconfig_name].attrs[
    ...
    if nboards > 0:
        tbx.qprint(quiet, 'SIS {0} boards used = {1}'.format(
            sid, sis_board_numbers))
    else:
    ...
    for ii in range(nboards):
        ...
        board_config_name = 'SIS crate {0} configurations[{1}]'.format(
            sid, iconfig)
        ...
        if sid == 3302:
            ...
            sample_averaging = 2**ff[board_config_group].attrs[
        ...
        if sid == 3305:
            ...
            # in sec, (4chan,2chan,1chan per FPGA)
            mode_index_to_dt = [8e-10, 4e-10, 2e-10]
        ...
        tbx.qprint(quiet, 'SIS {0} effective clock rate: Board {1} = {2:.1f} '
                   'MHz'.format(sid, iboard+1, 1/(1e6*dt)))

        nt = ff[board_config_group].attrs['Samples']
        time_acquired = nt * dt  # sec
        shots_averaged = ff[board_config_group].attrs[
        ...
        def config_attrs(attribute, sid, ichan):
            if sid == 3302:
                return ff[board_config_group].attrs[attribute+' '+str(ichan+1)]
            if sid == 3305:
                # helper function
                fpga_id, fpga_ch = get_sis3305_fpga_id(ichan)
                
                if attribute == 'Ch':
                    return ff[board_config_group].attrs[
                        'FPGA {0} {1} {2}'.format(fpga_id, attribute, fpga_ch)
                        ] + (fpga_id-1)*4
                else:
                    return ff[board_config_group].attrs[
                        'FPGA {0} {1} {2}'.format(fpga_id, attribute, fpga_ch)
        ...
        for jj in range(8):
            ...
            if sid == 3302:
                dc_offset[jj] = config_attrs('DC offset', sid, jj)
            else:
                dc_offset[jj] = None
    ...
    key_test = ['board_number', 'dt', 'nt', 'time_acquired', 'shots_averaged',
                'sample_averaging', 'bandwidth', 'channel_number', 'data_type',
                'comment', 'dc_offset']
    ...
    for jboard in temp:
        for jj in range(8):
            if jboard['channels_enabled'][jj] == 'TRUE':
                ...
                chan_info_temp['time_acquired'].append(
                    jboard['time_acquired'])
                chan_info_temp['shots_averaged'].append(
                    jboard['shots_averaged'])
                chan_info_temp['sample_averaging'].append(
                    jboard['sample_averaging'])
                chan_info_temp['bandwidth'].append(jboard['bandwidth'])
                chan_info_temp['channel_number'].append(
                    jboard['channel_number'][jj])
...
def read_sisboard_shot(fname, sisid, config_name, iboard=1, ichan=1, index=0):
    ...
    if sisid == 3302:
        ...
        dataset_config_name = config_name + config_subgroup_list[0] + \
            ' [Slot {0}: SIS 3302 ch {1}]'.format(islot, ichan)
        ...
        dataset_config_name = config_name + config_subgroup_list[0] + \
            ...
            format(islot, fpga_id, fpga_ch)

    # read file data
    ff = h5py.File(fname, 'r')
    ll = ff[dataset_config_name]  # data contained as (nshots, dt)
    print('Reading config name',dataset_config_name)
...
def read_msi_info(fname, quiet=0):
    ...
    #   ['Discharge', 'Gas pressure', 'Heater', 'Interferometer array',
    #   'Magnetic field']
    ...
    def unload_dataset_info(fname, name):
        ...
        for ii in list(ff[name].attrs):
            # unload root
            info_arr[ii] = ff[name].attrs[ii]

        for jj in dataset_list:
            # unload sub-directories
            info_arr[jj] = np.array(ff[name + jj + '/'])
...
def get_sis3305_fpga_id(ichan): # ichan = 5 should be fpga 2 ch1 ichan = 1 should be fpga1 cha 1
    chan_to_fpga_id = [1, 1, 1, 1, 2, 2, 2, 2]
    chan_to_fpga_chan = [1, 2, 3, 4, 1, 2, 3, 4]
    return chan_to_fpga_id[ichan-1], chan_to_fpga_chan[ichan-1]
