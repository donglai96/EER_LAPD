...
for s in range(9):
    data = read_lapd_data(filename, rchan=[1], rshot=[s], xrange=[20], yrange = [8]) 
    ...
    plt.semilogy(adjusted_freq/1e9/0.8538, smoothed_emf_fft, '.',label = 'shot = '+str(int(s)))
...
plt.legend()
plt.savefig('tst_ring_11s.png',format = 'png')
plt.show()